...
- **Chrome（Android / iPhone）:** Chromeのモバイル版は拡張機能をインストールできないため、このChrome拡張は実行できません。
- **Safari（iPhone / iPad）:** Safari Web ExtensionとしてiOSアプリにパッケージし直せば対応できます。ただし、現在の`extension/dist`をそのままインストールすることはできません。Xcodeでの変換、`chrome.offscreen` を使うローカル推論処理の互換実装、実機でのWebGPU・メモリ検証が必要です。
- **UI:** ポップアップの画面は360px幅を基準に設計し、360px / 390pxのモバイル幅で横スクロールが出ないことを確認しています。ページ上の操作検知はPointer Eventsを利用するため、Safari移植時にもタッチ操作を扱えます。
...
- Chrome ウェブストアへの提出資料は [`extension/store/`](extension/store/) にまとめています。ストアで利用可能になるのは、審査承認後です。
